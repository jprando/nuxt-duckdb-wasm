...
const COR_ERRO = "#f43f5e";
const PALETA = [COR_PRIMARIA, COR_SECUNDARIA, COR_TERCIARIA, COR_QUATERNARIA, COR_ERRO, "#14b8a6"];
...
export const useDiffusionDB = () => {
  ...
  const carregarDados = () => {
    ...
    executar(diffusionDBKpisConsulta(url))
      .then(([row]) => {
    ...
    executar(diffusionDBDimensoesConsulta(url))
      .then((data) => {
    ...
    executar(diffusionDBNsfwConsulta(url))
      .then((data) => {
        opcaoNsfw.value = {
          backgroundColor: "transparent",
          color: PALETA_NSFW,
    ...
    executar(diffusionDBStepsConsulta(url))
      .then((data) => {
        const rows = data as any[];
        opcaoSteps.value = {
          ...baseChart,
    ...
    executar(diffusionDBSamplerConsulta(url))
      .then((data) => {
        const rows = data as any[];
    ...
    executar(diffusionDBAtividadeHorariaConsulta(url))
      .then((data) => {
        const rows = data as any[];
